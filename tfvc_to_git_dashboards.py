...
def map_teams(source_teams, target_teams):
    ...
    for source_team in source_teams:
        ...
        if target_team_id: # Managed to find a corresponding team.
            ...
            print(f"[WARNING] No matching target team for source Team '{source_team_name}'!")
    ...
    if "children" in folder and folder["children"]:
        for child in folder["children"]:
            if child.get("isFolder", False):
                ...
                shared_queries.extend(extract_shared_queries(child))
            else:
...
def get_shared_queries(organization, project_name, authentication_header):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # As only shared queries are migrated, only they will be extracted.
            ...
            print(f"[INFO] Found {len(shared_queries)} shared queries.")
            
            for idx, query in enumerate(shared_queries):
                print(f"{idx + 1}. [INFO] Query ID: {query["id"]} | Name: {query["name"]} | Path: {query["path"]}")
...
def map_queries(source_queries, target_queries):
    ...
    try:
        ...
        for source_id, target_id in query_mapping.items():
            print(f"[INFO] Source Query ID: {source_id} -> Target Query ID: {target_id}")

        print(f"\n[INFO] {len(query_mapping)} queries mapped successfully.")
...
def create_dashboard(organization, project_name, team_name, dashboard_payload, authentication_header):
    ...
    try:
        ...
        if response.status_code == 200 or response.status_code == 201: # 201 status code indicates that the request was successfully 
            ...
            print(f"[INFO] Dashboard '{dashboard_payload["name"]}' created successfully.")
            return response.json()
        
        else:
            print(f"[ERROR] Failed to create dashboard '{dashboard_payload["name"]}' for team '{team_name}'.")
            print(f"[DEBUG] Response Status Code: {response.status_code}")
            print(f"[DEBUG] Response Body: {response.text}")
            return None
...
def create_widget(organization, project_name, team_name, dashboard_id, widget, authentication_header):
    ...
    print(f"[DEBUG] Widget Payload for '{widget["name"]}': {json.dumps(widget_payload, indent=4)}")
    ...
    try:
        ...
        if response.status_code == 200 or response.status_code == 201:
            print(f"[INFO] Widget '{widget["name"]}' added successfully to dashboard ID '{dashboard_id}'.")
            return response.json()
        
        else:
            print(f"[ERROR] Failed to add widget '{widget["name"]}' to dashboard ID '{dashboard_id}'.")
            print(f"[DEBUG] Response Status Code: {response.status_code}")
            print(f"[DEBUG] Response Body: {response.text}")
            return None
    ...
    try:
        ...
        for source_team in source_teams:
            ...
            if not target_team_id:
                print(f"[WARNING] Skipping team '{source_team_name}': No matching team found in target.")
                continue
            ...
            for source_dashboard in source_dashboards:
                ...
                if source_dashboard["name"] in target_dashboard_names:
                    print(f"[WARNING] A dashboard with the name '{source_dashboard["name"]}' already exists in the target project.")
                    user_input = input(f"Do you want to append the team name to the dashboard '{source_dashboard["name"]}
                                       ' ({source_dashboard["name"]}-{source_team_name})? (yes/no): ").strip().lower()
                ...
                for widget in widgets:
                    print(f"[DEBUG] Processing widget '{widget["name"]}'...")
                    ...
                    if source_query_id:
                        ...
                        if target_query_id:
                            ...
                            print(f"[WARNING] No target query found for source query ID {source_query_id}.")
        ...
        print("\n[INFO] Dashboard migration process complete.")
